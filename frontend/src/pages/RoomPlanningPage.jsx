...
import {
  ...
  CheckCircle, Settings, Filter, Users
} from 'lucide-react';
import {
  DndContext,
  PointerSensor,
...
// ─── Tooltip ─────────────────────────────────
function SessionTooltip({ session, color }) {
  const st = new Date(session.start_time);
  const et = new Date(session.end_time);
  const duration = ((et - st) / 3600000).toFixed(1);
  return (
    <div style={{
      ...
      <div style={{ fontWeight: 800, fontSize: 13, color, marginBottom: 6 }}>{session.training_title}</div>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 5, opacity: 0.85 }}>
          <Clock size={11} /> {st.toLocaleTimeString('fr-FR', { hour:'2-digit', minute:'2-digit' })} – {et.toLocaleTimeString('fr-FR', { hour:'2-digit', minute:'2-digit' })} ({duration}h)
        </div>
        {session.trainer_name && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 5, opacity: 0.85 }}>
            <User size={11} /> {session.trainer_name}
          </div>
        )}
        {session.room_name && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 5, opacity: 0.85 }}>
            <MapPin size={11} /> {session.room_name}
          </div>
        )}
        {session.enrolled_count != null && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 5, opacity: 0.85 }}>
            <Users size={11} /> {session.enrolled_count} étudiant(s) inscrit(s)
          </div>
        )}
        {session.notes && (
          <div style={{ marginTop: 4, opacity: 0.65, fontStyle: 'italic', fontSize: 11 }}>
            {session.notes}
          </div>
        )}
      </div>
      ...
      <div style={{
        ...
        width: 10, height: 10, background: 'var(--color-surface, #1c1b19)',
...
// ─── Session Block (draggable + resizable + tooltip) ─────
function SessionBlock({ session, roomColor, onEdit, onDelete, onComplete, onResizeEnd, isDragging: externalDragging }) {
  ...
  const resizeCurrentEnd = useRef(0);
  const [resizeEndH, setResizeEndH] = useState(null);
  ...
  const displayEndH  = resizeEndH !== null ? resizeEndH : Math.min(endH, HOUR_END);
  const clampedEnd   = Math.min(displayEndH, HOUR_END);
  const top    = (clampedStart - HOUR_START) * HOUR_HEIGHT;
  const height = Math.max((clampedEnd - clampedStart) * HOUR_HEIGHT - 4, 28);

  const color = session.room_color || roomColor || '#4f98a3';
  const sStyle = STATUS_STYLE[session.status] || STATUS_STYLE.planned;
  const isBeingDragged = isDragging || externalDragging;

  // Tooltip hover avec délai
  ...
  const handleResizeMouseDown = (e) => {
    ...
    resizeStartEnd.current = endH;
    resizeCurrentEnd.current = endH;

    const onMove = (ev) => {
      const delta = (ev.clientY - resizeStartY.current) / HOUR_HEIGHT;
      const raw = resizeStartEnd.current + delta;
      const snapped = Math.round(raw * 4) / 4; // snap 15min
      const clamped = Math.min(Math.max(snapped, clampedStart + 0.25), HOUR_END);
      resizeCurrentEnd.current = clamped;
      setResizeEndH(clamped);
    };
    const onUp = () => {
      setResizing(false);
      setResizeEndH(null);
      ...
      if (resizeCurrentEnd.current !== endH) {
        onResizeEnd(session, resizeCurrentEnd.current);
      }
    };
  ...
  return (
    <div
      ...
      style={{
        ...
        padding: '4px 7px',
        overflow: 'visible',
        cursor: resizing ? 'ns-resize' : (isBeingDragged ? 'grabbing' : 'grab'),
        zIndex: isBeingDragged ? 1000 : 2,
        opacity: isBeingDragged ? 0.4 : sStyle.opacity,
        transition: resizing || isBeingDragged ? 'none' : 'box-shadow 0.15s, transform 0.1s',
        boxShadow: hover ? `0 4px 16px ${color}44` : '0 1px 4px rgba(0,0,0,0.08)',
        transform: hover && !isBeingDragged && !resizing ? 'scale(1.012)' : 'scale(1)',
        userSelect: 'none',
      }}
      {...listeners}
      {...attributes}
    >
      {/* Tooltip */}
      {showTooltip && !isBeingDragged && !resizing && (
        <SessionTooltip session={session} color={color} />
      )}

      <div style={{ fontSize: 11, fontWeight: 700, color, lineHeight: 1.3, marginBottom: 1 }}>
        {formatHourDecimal(startH)} – {formatHourDecimal(resizeEndH !== null ? resizeEndH : endH)}
      </div>
      ...
      {hover && !isBeingDragged && (
        <div style={{ position: 'absolute', top: 3, right: 3, display: 'flex', gap: 3, zIndex: 20 }}>
      ...
      {session.status === 'planned' && (
        ...
        >
          <div style={{ width: 24, height: 2, borderRadius: 2, background: `${color}aa` }} />
        </div>
      )}
    </div>
  );
...
// ─── Droppable Cell ───────────────────────────
function DroppableCell({ id, room, hour, targetDate, onClick }) {
  const { setNodeRef, isOver } = useDroppable({ id, data: { room, hour, targetDate } });
  return (
    <div
      ref={setNodeRef}
      style={{
        ...
        left: 0, right: 0,
        height: HOUR_HEIGHT,
        background: isOver ? 'rgba(79,152,163,0.12)' : 'transparent',
        transition: 'background 0.15s',
        cursor: 'cell',
        zIndex: 1,
      }}
      onClick={onClick}
      title={`Programmer dans ${room?.name || ''} à ${String(hour).padStart(2,'0')}:00`}
    />
  );
}

// ─── Drag Overlay Ghost ───────────────────────
function DragGhost({ session, roomColor }) {
  if (!session) return null;
  const color = session.room_color || roomColor || '#4f98a3';
  const st = new Date(session.start_time);
  const et = new Date(session.end_time);
  const dur = (et - st) / 3600000;
  return (
    <div style={{
      width: 160, height: Math.max(dur * HOUR_HEIGHT - 4, 40),
      background: `${color}33`,
      border: `2.5px solid ${color}`,
      borderLeft: `4px solid ${color}`,
      borderRadius: 7,
      ...
      <div style={{ fontSize: 11, fontWeight: 700, color }}>
        {st.toLocaleTimeString('fr-FR', { hour:'2-digit', minute:'2-digit' })} – {et.toLocaleTimeString('fr-FR', { hour:'2-digit', minute:'2-digit' })}
      </div>
...
function CurrentTimeLine() {
  ...
  useEffect(() => {
    const update = () => {
      ...
      setLabel(`${String(now.getHours()).padStart(2,'0')}:${String(now.getMinutes()).padStart(2,'0')}`);
      if (h >= HOUR_START && h <= HOUR_END) setTop((h - HOUR_START) * HOUR_HEIGHT);
      else setTop(null);
    };
    update();
...
function SessionModal({ open, onClose, onSave, rooms, trainings, trainers, initialData }) {
  ...
  const handleSave = async () => {
    ...
    if (endISO <= startISO) { setError('L\'heure de fin doit être après l\'heure de début.'); return; }
    setSaving(true);
    try {
      await onSave({ id: initialData?.id, training_id: Number(form.training_id), trainer_id: Number(form.trainer_id), room_id: Number(form.room_id), start_time: startISO, end_time: endISO, notes: form.notes || null });
      onClose();
    } catch (e) {
        ...
        <div className="grid grid-2 gap-1 mb-1">
          ...
          <div>
            ...
            <select className="input w-full" value={form.trainer_id} onChange={e => set('trainer_id', e.target.value)}>
              <option value="">— Choisir —</option>
              {trainers.map(t => <option key={t.id} value={t.id}>{t.user?.first_name} {t.user?.last_name}</option>)}
            </select>
          </div>
          <div>
            ...
            <select className="input w-full" value={form.room_id} onChange={e => set('room_id', e.target.value)}>
              <option value="">— Choisir —</option>
              {rooms.map(r => <option key={r.id} value={r.id}>{r.name}{r.capacity ? ` (${r.capacity}p)` : ''}</option>)}
            </select>
          </div>
          <div className="col-span-2">
...
function RoomManagerPanel({ rooms, onRefresh, onClose }) {
  ...
  const handleUpdate = async (id) => {
    setSaving(true); setError('');
    try { await roomsService.update(id, { name: editName, capacity: editCap ? Number(editCap) : null, color: editColor }); setEditId(null); onRefresh(); }
    catch (e) { setError(e?.response?.data?.detail || 'Erreur.'); }
    finally { setSaving(false); }
  };
  ...
  return (
    <div className="card" style={{ width: 300, flexShrink: 0, padding: '1.25rem', height: '100%', overflowY: 'auto' }}>
      <div className="flex justify-between items-center mb-2">
        <h2 className="font-bold flex items-center gap-0-5"><Settings size={18} /> Salles</h2>
        <button className="btn-icon" onClick={onClose}><X size={18} /></button>
      </div>
      <div className="flex flex-col gap-0-75 mb-2">
        {rooms.length === 0 && <p className="text-sm text-muted text-center py-2">Aucune salle.</p>}
        {rooms.map(r => (
            ...
            {editId === r.id ? (
              <div className="flex flex-col gap-0-5">
                ...
                <div className="flex gap-0-5">
                  ...
                  <input type="color" value={editColor} onChange={e => setEditColor(e.target.value)} style={{ width:36, height:36, border:'none', borderRadius:6, cursor:'pointer', padding:2 }} />
                </div>
                <div className="flex gap-0-5">
              ...
              <div className="flex justify-between items-center">
                ...
                <div className="flex gap-0-5">
                  <button className="btn-icon" onClick={() => startEdit(r)}><Pencil size={14} /></button>
                  <button className="btn-icon" onClick={() => handleDelete(r.id)} style={{ color:'#ef4444' }}><Trash2 size={14} /></button>
                </div>
              </div>
            )}
          </div>
...
// ─── Filter Bar ───────────────────────────────
function FilterBar({ trainings, trainers, filterTraining, filterTrainer, onFilterTraining, onFilterTrainer, onClear }) {
  const hasFilter = filterTraining || filterTrainer;
  return (
    <div className="flex items-center gap-0-75 flex-wrap">
      ...
      <select
        ...
        onChange={e => onFilterTraining(e.target.value || null)}
      >
      ...
      <select
        ...
        style={{ minWidth: 160 }}
        value={filterTrainer || ''}
        onChange={e => onFilterTrainer(e.target.value || null)}
      >
        <option value="">Tous les formateurs</option>
        {trainers.map(t => <option key={t.id} value={t.id}>{t.user?.first_name} {t.user?.last_name}</option>)}
      </select>
      {hasFilter && (
        <button className="btn btn-ghost text-sm flex items-center gap-0-5" onClick={onClear}>
...
export default function RoomPlanningPage() {
  ...
  const fetchSessions = useCallback(async () => {
    const params = {};
    if (viewMode === 'day') params.date = toDateStr(currentDate);
    else params.week_start = toDateStr(getMonday(currentDate));
  ...
  useEffect(() => { fetchSessions(); }, [currentDate, viewMode]);
  ...
  const filteredSessions = sessions.filter(s => {
    if (filterTraining && String(s.training_id) !== String(filterTraining)) return false;
    if (filterTrainer  && String(s.trainer_id)  !== String(filterTrainer))  return false;
    return true;
  });
  ...
  const handleCellClick = (room, hour, targetDate) => {
    ...
    setModal({ open: true, data: {
      room_id: room?.id,
      date: d,
      start_hour: String(Math.floor(hour)).padStart(2,'0'),
      start_min: hour % 1 === 0.5 ? '30' : '00',
      end_hour: String(Math.floor(hour) + 2).padStart(2,'0'),
      end_min: '00',
    }});
  };
  ...
  // Resize : recalcule end_time et sauvegarde
  const handleResizeEnd = async (session, newEndH) => {
    const st = new Date(session.start_time);
    const dateStr = toDateStr(st);
    const endHH = Math.floor(newEndH);
    const endMM = Math.round((newEndH % 1) * 60);
    const newEndISO = `${dateStr}T${String(endHH).padStart(2,'0')}:${String(endMM).padStart(2,'0')}:00`;
    try {
      await sessionsService.updatePlannedSession(session.id, {
        ...session,
        end_time: newEndISO,
      });
      showToast('Durée mise à jour.');
      fetchSessions();
    } catch { showToast('Erreur lors du redimensionnement.', 'error'); }
  };

  // DnD handlers
  const handleDragStart = ({ active }) => {
    const s = active.data.current?.session;
    setActiveDragSession(s || null);
  };
  ...
  const handleDragEnd = async ({ active, over }) => {
    ...
    const { room, hour, targetDate } = over.data.current || {};
    if (!session || !room || hour == null) return;

    const st = new Date(session.start_time);
    const et = new Date(session.end_time);
    const duration = (et - st) / 3600000; // heures

    const dateStr = toDateStr(targetDate || currentDate);
    const startHH = Math.floor(hour);
    const startMM = Math.round((hour % 1) * 60);
    const endH = hour + duration;
    const endHH = Math.floor(endH);
    const endMM = Math.round((endH % 1) * 60);

    const newStart = `${dateStr}T${String(startHH).padStart(2,'0')}:${String(startMM).padStart(2,'0')}:00`;
    const newEnd   = `${dateStr}T${String(endHH).padStart(2,'0')}:${String(endMM).padStart(2,'0')}:00`;
  ...
  const weekDays = (() => {
    ...
    return Array.from({ length: 6 }, (_, i) => { const d = new Date(mon); d.setDate(d.getDate() + i); return d; });
  })();
  ...
  const renderDayGrid = () => (
    <div style={{ display:'flex', minWidth:0 }}>
      <div style={{ width:52, flexShrink:0, position:'relative', height:TOTAL_H+1 }}>
        {HOURS.map(h => (
          <div key={h} style={{ position:'absolute', top:(h-HOUR_START)*HOUR_HEIGHT-9, right:6, fontSize:11, fontWeight:600, color:'#9ca3af', userSelect:'none' }}>
      ...
      {rooms.map(room => {
        const roomSessions = filteredSessions.filter(s =>
          (s.room_id === room.id || s.room_name === room.name) &&
          s.start_time &&
          new Date(s.start_time).toISOString().split('T')[0] === toDateStr(currentDate)
        );
        return (
          <div key={room.id} style={{ flex:1, minWidth:120, borderLeft:'1px solid #e5e7eb', position:'relative', height:TOTAL_H }}>
            {HOURS.map(h => (
              <React.Fragment key={h}>
                <div style={{ position:'absolute', top:(h-HOUR_START)*HOUR_HEIGHT, left:0, right:0, height:HOUR_HEIGHT, borderBottom:'1px solid #f3f4f6', pointerEvents:'none' }}>
                  <div style={{ position:'absolute', top:HOUR_HEIGHT/2, left:0, right:0, borderBottom:'1px dashed #f3f4f6' }} />
                </div>
                <DroppableCell
                  id={`drop-${room.id}-${h}`}
                  room={room} hour={h} targetDate={currentDate}
            ...
            {roomSessions.map(s => (
              <SessionBlock
                key={s.id} session={s} roomColor={room.color}
                onEdit={s => setModal({ open:true, data:s })}
                onDelete={handleDeleteSession}
                onComplete={handleCompleteSession}
                onResizeEnd={handleResizeEnd}
                isDragging={activeDragSession?.id === s.id}
              />
            ))}
          </div>
        );
        ...
        <div key={toDateStr(day)} style={{ flex:1, minWidth:90, borderLeft:'1px solid #e5e7eb', position:'relative', height:TOTAL_H }}>
          {HOURS.map(h => (
            <React.Fragment key={h}>
              ...
              <DroppableCell
                id={`drop-${toDateStr(day)}-${h}`}
                room={null} hour={h} targetDate={day}
                onClick={() => handleCellClick(null, h, day)}
              />
            </React.Fragment>
          ))}
          <CurrentTimeLine />
      ...
      <div className="fade-in" style={{ display:'flex', flexDirection:'column', gap:'1rem' }}>
        ...
        <div className="flex justify-between items-start flex-wrap gap-1">
          <div>
            ...
            <p className="text-muted">Glissez-déposez les séances, étirez pour ajuster la durée.</p>
          </div>
          <div className="flex items-center gap-0-75">
            <button className="btn btn-secondary flex items-center gap-0-5 text-sm" onClick={() => setShowPanel(v => !v)}>
              <Settings size={15} /> Salles
            </button>
            <button className="btn btn-primary flex items-center gap-0-5 text-sm" onClick={() => setModal({ open:true, data:null })}>
        ...
        <div style={{ display:'flex', gap:'1rem', alignItems:'flex-start' }}>
          {showPanel && <RoomManagerPanel rooms={rooms} onRefresh={fetchMeta} onClose={() => setShowPanel(false)} />}
          ...
          <div style={{ flex:1, minWidth:0 }}>
            ...
            <div className="flex items-center gap-1 mb-1 flex-wrap">
              <div className="flex rounded-lg overflow-hidden" style={{ border:'1px solid var(--border,#e5e7eb)' }}>
                {['day','week'].map(v => (
                  <button key={v}
                    className={`px-3 py-1 text-sm font-bold transition-all ${viewMode === v ? 'btn-primary' : 'bg-glass'}`}
                    style={{ border:'none', cursor:'pointer' }}
                    onClick={() => setViewMode(v)}
                  >{v === 'day' ? 'Jour' : 'Semaine'}</button>
                ))}
              </div>
            ...
            <div className="mb-1">
              <FilterBar
                trainings={trainings} trainers={trainers}
                filterTraining={filterTraining} filterTrainer={filterTrainer}
                onFilterTraining={setFilterTraining} onFilterTrainer={setFilterTrainer}
                onClear={() => { setFilterTraining(null); setFilterTrainer(null); }}
              />
            </div>
            ...
            <div className="card p-0 overflow-x-auto" style={{ borderBottom:'2px solid var(--border,#e5e7eb)' }}>
              <div style={{ display:'flex' }}>
                ...
                {viewMode === 'day'
                  ...
                  : weekDays.map(day => (
                      <div key={toDateStr(day)} style={{ flex:1, minWidth:90, borderLeft:'1px solid #e5e7eb', padding:'0.5rem', textAlign:'center', background: isToday(day) ? 'rgba(79,152,163,0.08)' : 'transparent' }}>
                        <div style={{ fontWeight:700, fontSize:12, textTransform:'capitalize' }}>{day.toLocaleDateString('fr-FR', { weekday:'short' })}</div>
                        <div style={{ fontWeight: isToday(day) ? 800 : 600, fontSize:16, color: isToday(day) ? '#4f98a3' : 'inherit' }}>{day.getDate()}</div>
                      </div>
                    ))
                }
              </div>
        ...
        <div className="grid grid-3 gap-1-5">
          <div className="card flex items-center gap-1 p-1-5">
            <div className="p-1 rounded-lg" style={{ background:'rgba(79,152,163,0.12)', color:'#4f98a3' }}><Clock size={22} /></div>
            <div>
          ...
          <div className="card flex items-center gap-1 p-1-5">
            <div className="p-1 rounded-lg" style={{ background:'rgba(122,90,248,0.12)', color:'#7a5af8' }}><MapPin size={22} /></div>
            <div>
          ...
          <div className="card flex items-center gap-1 p-1-5">
            <div className="p-1 rounded-lg" style={{ background:'rgba(16,185,129,0.12)', color:'#10b981' }}><Calendar size={22} /></div>
            <div>
        ...
        <SessionModal
          ...
          rooms={rooms} trainings={trainings} trainers={trainers}
      ...
      <DragOverlay dropAnimation={null}>
        {activeDragSession && (
          <DragGhost
            ...
            roomColor={rooms.find(r => r.id === activeDragSession.room_id)?.color}
          />
        )}
      </DragOverlay>
    </DndContext>
